exports.run = async (client, message, [credit]) => {
    ...
    db.get(`SELECT * FROM scores WHERE userId = "${message.author.id}"`, [], (err, row) => {
        ...
        if (sum < 4) { var results = 1; } 
            ...
            do {
                if (castMain === sumAll[i]) { var results = 0; loop = false;  } 
                else if (chance === sumAll[i]) { var results = 1; loop = false; } 
                else if (i === 4) { var results = 1; loop = false; }
                else { i += 1; } 
            } while (loop === true);
        }
        var result = 0 ? "won" : "lost"; var earnings;
        ...
        if (result === "won" && chance > 3) {
            ...
            earnings = (credit * earnings).toFixed(0);
        } else if (result === "won") { earnings = (credit * 2); } 
        else if (result === "lost") { earnings = (credit * -1); }
        ...
        if (sum[0] > 10) { embed.addField(`Rolled higher than a 10! Sum and caster numbers determine you ${result}!`); }
