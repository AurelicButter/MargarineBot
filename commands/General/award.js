exports.run = async (client, message, [user, type, ...text]) => {
    ...
    if (user !== undefined && message.author.id === client.owner.id) {
        ...
        var data = await client.funcs.userSearch(client, message, {user: [user], tags:["bot"], name: this.help.name});
        ...
        db.get(`SELECT credits FROM scores WHERE userId = "${data.user[0].id}"`, [], (err, row) => {
            ...
            client.users.get(data.user[0].id).then(avatar => {
                embed.setTitle(":tada: Award Notification! :tada:")
                    ...
                    .setFooter("Awarded to: " + data.user[0].tag + " (" + data.user[0].id + ") on:", avatar.displayAvatarURL());
